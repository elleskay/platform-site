...
function Btn({ href, children, fill = "green" }: { href: string; children: React.ReactNode; fill?: "green" | "ink" | "paper" }) {
  const bg = fill === "green" ? "bg-[var(--color-green-fill)]" : fill === "ink" ? "bg-[var(--color-ink)] text-[var(--color-bg)]" : "bg-[var(--color-paper-card)]";
  return (
    ...
    <header className="sticky top-0 z-50 border-b-2 border-[var(--color-ink)] bg-[var(--color-bg)]">
      <nav className="mx-auto flex max-w-6xl items-center gap-4 px-5 py-3">
        <a href="#top" className="flex items-center gap-2.5">
          <span className={`${INK} bg-[var(--color-green-fill)] flex h-9 w-9 items-center justify-center`}><Mark className="h-5 w-5" /></span>
        ...
        <a href={REPO} className={`mono ${INK} bg-[var(--color-ink)] hard-sm press ml-auto px-4 py-2 text-sm font-bold text-[var(--color-bg)] md:ml-0`} style={{ boxShadow: "4px 4px 0 0 var(--color-ink)" }}>
          use template
        </a>
      </nav>
    </header>
  ...
  return (
    <section id="top" className="relative overflow-hidden border-b-2 border-[var(--color-ink)]">
      ...
      <div className="relative mx-auto max-w-6xl px-5 py-16 sm:py-24">
        <span className={`mono ${INK} bg-[var(--color-yellow)] hard-sm inline-flex items-center gap-2 px-3 py-1.5 text-xs font-bold uppercase tracking-[0.14em]`} style={{ boxShadow: "4px 4px 0 0 var(--color-ink)" }}>
          a template, not a product
        </span>
        <h1 className="mt-7 max-w-4xl text-[44px] font-bold leading-[0.92] tracking-tight sm:text-[80px]">
          The boring parts of{" "}
          <span className="mono inline-block -rotate-1 border-2 border-[var(--color-ink)] bg-[var(--color-green-fill)] px-2.5 leading-tight">shipping</span>,
          <br className="hidden sm:block" /> already{" "}
          <span className="mono inline-block rotate-1 border-2 border-[var(--color-ink)] bg-[var(--color-purple-fill)] px-2.5 leading-tight">solved</span>.
        </h1>
...
function CellHead({ tag, color }: { tag: string; color: string }) {
  return (
    <div className="mono mb-3 flex items-center gap-2 text-[12px] font-bold uppercase tracking-[0.14em] text-[var(--color-faint)]">
      <span className="h-2.5 w-2.5 border border-[var(--color-ink)]" style={{ background: color }} /> {tag}
    </div>
  );
}
    ...
    <section id="grid" className="border-b-2 border-[var(--color-ink)]">
      <div className="mx-auto max-w-6xl px-5 py-16 sm:py-24">
        <h2 className="max-w-2xl text-3xl font-bold leading-[1.05] tracking-tight sm:text-5xl">
          One clone, the whole <span className="mono border-2 border-[var(--color-ink)] bg-[var(--color-yellow)] px-2">stack</span>.
        </h2>
...
`}<span className="text-[var(--color-green)]">{`// Lambda + S3 + CloudFront, every gotcha baked in`}</span>
          ...
          <div className="flex flex-col gap-4">
            <div className={`${INK} hard lift bg-[var(--color-green-fill)] flex-1 p-5`}>
              <CellHead tag="infra" color="var(--color-paper-card)" />
            ...
            <div className={`${INK} hard lift bg-[var(--color-purple-fill)] flex-1 p-5`}>
              <CellHead tag="spec gate" color="var(--color-paper-card)" />
        ...
        <div className="mt-4 grid gap-4 md:grid-cols-3">
          ...
          <div className={`${INK} hard lift bg-[var(--color-yellow)] p-5`}>
            <CellHead tag="smoke test" color="var(--color-paper-card)" />
        ...
        <div className="grid items-center gap-10 lg:grid-cols-2">
          <div>
            <h2 className="text-3xl font-bold leading-[1.05] tracking-tight sm:text-5xl">
              The bill for <span className="mono inline-block -rotate-1 border-2 border-[var(--color-ink)] bg-[var(--color-green-fill)] px-2">not</span> using a template.
            </h2>
          ...
          <div className={`${INK} hard-lg bg-[var(--color-paper-card)]`}>
            ...
            <div className="mono flex items-baseline justify-between border-t-2 border-dashed border-[var(--color-ink)] px-6 py-4">
              ...
              <span className="border-2 border-[var(--color-ink)] bg-[var(--color-green-fill)] px-2 text-xl font-bold">git clone</span>
            </div>
          </div>
        </div>
      </div>
    ...
    <section className="relative overflow-hidden border-b-2 border-[var(--color-ink)] bg-[var(--color-green-fill)]">
      ...
      <div className="relative mx-auto max-w-3xl px-5 py-24 text-center">
        <h2 className="text-4xl font-bold leading-[1] tracking-tight sm:text-6xl">
          ...
          <span className="mono mt-3 inline-block border-2 border-[var(--color-ink)] bg-[var(--color-paper-card)] px-3">git clone</span>.
        </h2>
        ...
        <div className={`mono ${INK} hard-lg mx-auto mt-9 flex max-w-xl items-center gap-3 bg-[var(--color-ink)] px-5 py-4 text-left text-sm text-[var(--color-bg)]`}>
          ...
          <a href={REPO} className="mono press ml-auto shrink-0 border-2 border-[var(--color-bg)] bg-[var(--color-green-fill)] px-3 py-1.5 text-xs font-bold text-[var(--color-ink)]">open</a>
        </div>
      </div>
    </section>
  );
